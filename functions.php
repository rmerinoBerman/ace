...
// LOAD SCRIPTS
    ...
    function loadScripts() {
    	...
    	wp_register_script('dynamics', get_template_directory_uri() . '/machines/dynamics.js', array('timepicker'), rand(1, 10000) . '.' . rand(1, 10000), false );
...
// CUSTOM POST TYPE PEOPLE 
	...
	include(get_template_directory() . '/machines/functions/custom_post_people_type.php');

// CUSTOM POST TYPE SPONSORS
	include(get_template_directory() . '/machines/functions/custom_post_sponsorship.php');
	...
	function generatePagesJSON($currentPageID){
		...
		foreach ($allPageIDs as $value) {
			...
			if (has_post_thumbnail( $value ) ){
				$featuredImageArray = wp_get_attachment_image_src( get_post_thumbnail_id( $value ), 'single-post-thumbnail' );
				$featuredImageURL = $featuredImageArray[0];
			} else {
				$featuredImageURL = null;
			}
